...
const SupplierReport: React.FC = () => {
    ...
    const suppliers = ['Supplier A', 'Supplier B', 'Supplier C', 'Supplier D', 'All Suppliers'];
    ...
    const fetchReport = async (data: ReportFormData): Promise<ReportResult[]> => {
        ...
        try {
            ...
            const mockResults: ReportResult[] = Array.from({ length: 5 }, (_, i) => ({
                id: i + 1,
                supplier: data.supplier || suppliers[Math.floor(Math.random() * suppliers.length)],
    ...
    const exportToCSV = () => {
        ...
        const headers = 'ID,Supplier,Purchase Type,Date,Amount\n';
        const csvContent = reportResults
            .map(item => `${item.id},${item.supplier},${item.purchaseType},${item.date},${item.amount}`)
            .join('\n');
        const blob = new Blob([headers + csvContent], { type: 'text/csv' });
        downloadFile(blob, 'supplier_report.csv');
    };
                ...
                <form onSubmit={handleSearch} className="w-full">
                    <div className="grid grid-cols-1 md:grid-cols-5 gap-6">
                        <div className="flex flex-col">
                            <label htmlFor="supplier" className="text-sm font-medium text-gray-700 mb-1">
                                Supplier
                            </label>
                            <select
                                id="supplier"
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="bg-gray-100">
                                    ...
                                    <th className="p-3 border-b font-semibold text-gray-700">Supplier</th>
                            ...
                            <tbody>
                                {reportResults.map((result) => (
                                    <tr key={result.id} className="hover:bg-gray-50">
                                        ...
                                        <td className="p-3 border-b text-gray-600">{result.supplier}</td>
